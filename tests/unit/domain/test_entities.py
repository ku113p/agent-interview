...
from src.domain.entities.user import Career, EmailAddress, UserProfile
...
class TestUserProfile:
    def test_should_create_valid_user(self) -> None:
        user = UserProfile(
            ...
            full_name="Teo",
            career=Career(profession="Coder"),
        )
    ...
    def test_update_profession_should_return_new_instance(self) -> None:
        user = UserProfile(
            email=EmailAddress(value="career@example.com"),
            career=Career(profession="", experience_years=2),
        )
        updated_user = user.update_profession("Senior Dev", 5)

        assert user.profession == ""
